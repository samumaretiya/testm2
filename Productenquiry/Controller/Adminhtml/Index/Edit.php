...
class Edit extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if ($id) {
            $model->load($id);
            if (!$model->getId()) {
                $this->messageManager->addError(__('This productenquiry no longer exists.'));
        ...
        $resultPage->addBreadcrumb(
            $id ? __('Edit Productenquiry') : __('New Product Enquiry'),
            $id ? __('Edit Productenquiry') : __('New Product Enquiry')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Product Enquiry'));
        $resultPage->getConfig()->getTitle()
            ->prepend($model->getId() ? $model->getTitle() : __('New Product Enquiry'));
        return $resultPage;
    }
}
